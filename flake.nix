{
  ...
  outputs = { self, nixpkgs, utils, ops-lib, bitte, ... }@inputs:
    bitte.lib.simpleFlake {
      ...
      packages = { generate-mantis-keys, mantis-faucet-nginx
        , mantis-explorer-nginx, restic-backup, mantis-faucet-server, checkFmt
        , devShellPath }@pkgs:
        pkgs;
      ...
      hydraJobs = { bitte, cfssl, consul, cue, devShellPath, grafana
        , grafana-loki, haproxy, nixFlakes, nomad, sops, terraform-with-plugins
        , vault-bin, victoriametrics, generate-mantis-keys, mantis
        , mantis-staging, mantis-faucet-web, mantis-faucet-nginx
        , mantis-faucet-server, mantis-explorer, mantis-explorer-nginx
        , morpho-node, morpho-node-entrypoint }@jobs:
